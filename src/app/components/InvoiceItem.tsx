...
export default function InvoiceItem({ invoice }: { invoice: Invoice }) {
  ...
  return (
    <tr key={invoice.id}>
      <td className="border whitespace-nowrap text-left border-slate-200 p-4 pl-8 text-black ">
      ...
      <td className="border whitespace-nowrap text-left border-slate-200 p-4 pl-8 text-black ">{invoice.name} {invoice.lastname}</td>
      <td className="border whitespace-nowrap text-right border-slate-200 p-4 pl-8 text-black ">{(invoice.amount + invoice.iva - invoice.irpf) / 100} EUR</td>
      <td className="border whitespace-nowrap border-slate-200 p-4 pl-8 text-black ">{invoice.invoiceid}</td>
      <td className="border whitespace-nowrap border-slate-200 p-4 pl-8 text-black">
        <BadgeStatus status={invoice.status}/>
      </td>
      <td className="border whitespace-nowrap text-right border-slate-200 p-4 pl-8 text-black ">{date.toLocaleDateString('ca', {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      })}</td>
